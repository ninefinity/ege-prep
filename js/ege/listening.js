...
E.buildListeningAudio = function buildListeningAudio(task, topicId, options) {
    ...
    if (usePlayerLayout) {
      // Play/pause, the seek bar, and Notes all share one row -- a
      // separate row just for Notes (tried first) left the player
      // reading as a tall, padded card; one slim inline row plus
      // stripping the surrounding card look (see .ege-listening-bar in
      // shell.css) is what actually made it compact.
      ...
      if (actionsStart.childNodes.length) topRow.appendChild(actionsStart);

      notesSlot = document.createElement("div");
      notesSlot.className = "ege-listening-player__notes-slot";
      topRow.appendChild(notesSlot);
