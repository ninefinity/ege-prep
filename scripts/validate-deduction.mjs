#!/usr/bin/env node
// Brute-force uniqueness checker for data/writing-skills-facts.json's "Data
// Deduction" clue sets (the `deduction-*` tasks). For each task carrying a
// `_validate` block, tries every permutation of `_validate.percentages`
// against `_validate.options` and asserts that exactly one permutation
// satisfies every clue in `_validate.checks` — and that it's the one encoded
// in the task's `left[].match` / `right[]` pairing data. Run this whenever a
// task is added or edited:
//
